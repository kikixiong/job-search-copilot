...
每条 observation 记录 URL、source tier、retrieved-at、精确 locator、open/closed/lead、明确 deadline、canonical apply URL 和冲突。官方 closed observation 关闭职位；互相矛盾的官方证据必须显示 `conflict`，不能静默挑选有利结果。

## 平台边界

可读取公开可访问页面，但不登录、不导入 Cookie、不复用 session、不绕过访问控制、不解决 CAPTCHA/MFA，也不对 LinkedIn 或 Indeed 做自动抓取。Workday 和其他 ATS 的公开页面可作为只读证据；访问超时、403、limited 或 missing 时记录失败和时间，再尝试允许的官方替代来源。
...
Redirect、子域、lookalike、userinfo、敏感 query/fragment、LinkedIn、Indeed、Workday、公司自有未知域以及任何缺失/过期/冲突条件都降级为逐字段 copy。`manual_only` 始终包含 EEO/demographic、disability、veteran、consent、signature、CAPTCHA/MFA 和 final submit。
