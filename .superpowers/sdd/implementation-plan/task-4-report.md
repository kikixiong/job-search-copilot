...
## Scoped final re-review closure — batch projection and historical field reclassification

This section supersedes the schema-8 and 97-test closure evidence immediately above for the two Important portions found by the scoped re-review.

- `recordSearchBatch()` now projects each deduplicated survivor with the current input `runId`. A real two-run MCP regression records an official-open 99-point match in run A, then the same canonical opportunity as an unmatched community lead in run B. Both the run-B batch response and `opportunities_query({ runId: runB })` contain only the run-B observation, derive `community_lead`, and return `match: null`.
- Every Viewer opportunity score now displays its assessment `runId` beside the score, so a workspace-global recovery score cannot be mistaken for the newest search scope.
- Schema migration 9 selects every historical application field and calls the same current `classifyApplicationField(field_key, label)` used for new writes. It updates all classifications, clears values only when the resulting class is `manual_only`, and never replaces the stable field ID. Full v7 and v8 packet fixtures cover bare submit, `最终提交`, `同意条款`, and `电子签名`; safe and confirm reclassification is also verified. Review rejects acknowledgement of migrated manual-only IDs and accepts only the remaining confirm ID. A v6 fixture exercises the structural v7 migration followed by v8/v9, while the existing future-version rejection remains green.

Focused evidence: Core 35/35, MCP 15/15 with exactly 12 tools, and Viewer DOM 12/12. The consolidated gate then passed at 101/101 plus lint, typecheck, build, and tracked sensitive scanning. Viewer output was HTML 0.45 kB, CSS 10.37 kB, JS 215.62 kB (67.91 kB gzip). Release evidence and superseding artifact hashes are in the Task 5 scoped addendum. No dependency/lockfile, remote, publish, or no-submit boundary changed.
